...
::: warning
<<<<<<< HEAD
MAA 需要 Visual C++ Redistributable x64（VCRedist x64）和 .NET 8 。请在解压后的 MAA 目录中以管理员身份运行 `DependencySetup_依赖库安装.bat` 来安装这些依赖。安装完成后再运行 `maa.exe`。
=======
